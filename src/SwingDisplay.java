	...
	private void setupGUI() {
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// request trip
				System.out.println("noted action");//for debugging
				if (selections == 2) {
					System.out.println("making new trip");//for debugging
		...
		canvas.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if(selections == 0) {
					System.out.println("start found");//for debugging
					startX = e.getX();
					startY = e.getY();
					selections++;
				} else if (selections == 1) {
					System.out.println("end found");//for debugging
					endX = e.getX();
					endY = e.getY();
					selections++;
				}
